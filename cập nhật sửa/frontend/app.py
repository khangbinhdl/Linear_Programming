...
# ==========================================
# CẤU HÌNH ĐƯỜNG DẪN IMPORT (SỬA Ở ĐÂY)
# ==========================================
# Lấy thư mục hiện tại (thư mục 'frontend')
current_dir = os.path.dirname(os.path.abspath(__file__))
# Lấy thư mục gốc (thư mục cha của 'frontend', tức là 'Linear_Programming')
parent_dir = os.path.dirname(current_dir)
# Thêm thư mục gốc vào hệ thống để Python tìm thấy Simplex và Graphical
...
with tab1:
    ...
    for i in range(num_vars):
        ...
        obj_coeffs.append(parse_float(val_str))
    ...
    for i in range(num_cons):
        ...
        for j in range(num_vars):
            ...
            coeffs.append(parse_float(val_str))
        ...
        constraints_data.append((coeffs, sense, parse_float(rhs_str)))
...
with tab2:
    ...
    c1 = parse_float(obj_cols_g[2].text_input("c1", value="1", key="g_c1", label_visibility="collapsed"))
    obj_cols_g[3].markdown('<div class="inline-text">x<sub>1</sub> + </div>', unsafe_allow_html=True)
    c2 = parse_float(obj_cols_g[4].text_input("c2", value="1", key="g_c2", label_visibility="collapsed"))
    ...
    for i in range(int(num_cons_g)):
        ...
        a1 = parse_float(cg_cols[1].text_input("a1", value="1", key=f"ga1_{i}", label_visibility="collapsed"))
        cg_cols[2].markdown('<div class="inline-text">x<sub>1</sub> + </div>', unsafe_allow_html=True)
        a2 = parse_float(cg_cols[3].text_input("a2", value="1", key=f"ga2_{i}", label_visibility="collapsed"))
        ...
        rhs = parse_float(cg_cols[6].text_input("RHS", value="10", key=f"grhs_{i}", label_visibility="collapsed"))
